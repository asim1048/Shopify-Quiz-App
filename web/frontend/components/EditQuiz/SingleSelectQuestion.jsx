...
const SingleSelectQuestion = ({ question, setQuestions, index,backendURL }) => {
  ...
  const handleImageUpload = (event, optionIndex) => {
    ...
    setQuestions(prevQuestions => {
      ...
      const updatedOptions = questionToUpdate.options.map((option, i) =>
        i === optionIndex ? { ...option, image: file, newImage: true } : option
      );
      updatedQuestions[index] = { ...questionToUpdate, options: updatedOptions };
      return updatedQuestions;
    });
      ...
      {question?.options?.map((option, optionIndex) => (
        <div key={option.id} style={{ marginTop: '10px' }}>
          ...
          <div style={{ marginTop: '5px' }}>
            {option.newImage 
              ? <img src={URL.createObjectURL(option.image)} alt="Uploaded Preview" style={{ maxWidth: '100%', maxHeight: '200px', marginTop: '10px', borderRadius: "7px" }} />
              : option.image && <img src={`${backendURL}/${option.image}`} alt="Uploaded Preview" style={{ maxWidth: '100%', maxHeight: '200px', marginTop: '10px', borderRadius: "7px" }} />
            }
          </div>
